...
const Graph:React.FC<GraphProps> = ({shouldFetchData,onFetchComplete })=> {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        console.log("APi call for static graph begun")
        ...
        setData2([newData2]);
        setData3([newData3]);
        console.log("loggin after fetching static data");
        setLoading(false);
        onFetchComplete();
      } catch (error) {
